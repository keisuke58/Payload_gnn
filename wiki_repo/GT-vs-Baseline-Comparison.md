...
| **セクタ角** | 30° (1/12) | 60° (1/6) |
| **メッシュシード** | 10mm (量産用) / 25mm (テスト) | ~15mm (推定, 25mm 指定だがリファイン有) |
| **OuterSkin ノード** | **143,184** (seed=10) / 21,243 (seed=25) | **~120,000** (median) |
| **OuterSkin 要素** | ~142K (seed=10) / 20,924 (seed=25) | ~119,714 (S4R) |
...
## 6. 高密度メッシュ (seed=10mm) 検証

> **テスト日**: 2026-03-03
> **コマンド**: `--global_seed 10` (healthy)
> **実行環境**: frontale04 (93GB RAM, 24コア)

### 6.1 メッシュ密度比較

| 項目 | GT seed=25 | **GT seed=10** | Baseline |
|------|-----------|---------------|----------|
| GLOBAL_SEED | 25mm | **10mm** | ~25mm |
| OuterSkin ノード | 21,243 | **143,184** | ~120,000 |
| INP サイズ | 14MB | **127MB** | ~50MB |
| ODB サイズ | 209MB | **2.8GB** | ~100MB |
| メモリ使用 | ~3GB | **8.5GB** | — |
| 解析時間 | ~2分 | **~55分** | — |
| ノード/面積密度 | 低 | **高 (ベースライン比 1.2x)** | 基準 |

### 6.2 運用方針

- **GT は seed=10 で生成** — ground truth として高精度が重要
- ベースラインより 1.2 倍密 → GNN 検証用として十分
- ODB 2.8GB/サンプル → 20サンプルで ~56GB (ストレージ注意)
- 解析 55分/サンプル → 20サンプルで ~18時間 (frontale04 で並列可)
...
- [x] メッシュ密度調整: `--global_seed 10` → **143K ノード** — **完了** (2026-03-03)
- [ ] GT バッチ生成 (10-20 サンプル, seed=10) → 推論検証用データセット
- [ ] GNN 推論テスト: 学習済みモデル → GT データで精度評価
