...
MAE は binary (healthy/defect) で学習したが、マルチクラスラベル付きデータ
(`processed_s12_czm_thermal_200`, 8クラス) で評価。

**欠陥タイプ別検出精度** (val 40 graphs, 608K nodes):

| Type | ROC-AUC | PR-AUC | F1 | N nodes | 備考 |
|------|---------|--------|------|---------|------|
| **overall** | **0.992** | **0.769** | **0.775** | 3,693 | binary baseline |
| debonding | 1.000 | 0.761 | 0.740 | 885 | 主要ターゲット |
| impact | 1.000 | 0.777 | 0.782 | 492 | 最も検出しやすい |
| delamination | 1.000 | 0.789 | 0.754 | 557 | 応力異常が明確 |
| fod | 0.999 | 0.573 | 0.671 | 495 | 硬さ変化 |
| thermal_prog | 0.999 | 0.413 | 0.553 | 444 | CTE不整合 |
| acoustic_fat | 0.999 | 0.332 | 0.492 | 134 | サンプル少 |
| inner_debond | 0.957 | 0.016 | 0.042 | 686 | 内側面 → 外側応力への影響小 |

**教師なしクラスタリング** (k-means on physics dims of residual):
- NMI: 0.226, ARI: 0.146
- 部分的な分離構造あり、完全分離には至らず
