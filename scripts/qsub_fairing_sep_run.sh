...
# qsub_fairing_sep_run.sh — INP 再生成 + Abaqus/Explicit 解析
# Usage: qsub -v JOB_NAME=Sep-Test scripts/qsub_fairing_sep_run.sh
#        qsub -v JOB_NAME=Sep-Abnormal,EXTRA_ARGS="--n_stuck_bolts 3" scripts/qsub_fairing_sep_run.sh
...
# 古い結果ファイルを削除（INP も再生成するので削除）
rm -f "$WORK_DIR/$JOB_NAME.inp" "$WORK_DIR/$JOB_NAME.odb" \
      "$WORK_DIR/$JOB_NAME.sta" "$WORK_DIR/$JOB_NAME.msg" \
      "$WORK_DIR/$JOB_NAME.dat" "$WORK_DIR/$JOB_NAME.lck"

# Phase 1: INP 生成（セクション割り当て修正済み）
echo ""
...
if grep -q "COMPLETED SUCCESSFULLY" "$WORK_DIR/$JOB_NAME.sta" 2>/dev/null; then
    ...
    echo "  Last 20 lines of .sta:"
    tail -20 "$WORK_DIR/$JOB_NAME.sta" 2>/dev/null
    echo "  Last 30 lines of .dat:"
    tail -30 "$WORK_DIR/$JOB_NAME.dat" 2>/dev/null
    exit 1
fi
