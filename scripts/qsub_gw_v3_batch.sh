#!/bin/bash
# qsub_gw_v3_batch.sh — DOE JSON からv3 ジョブを一括投入 / 状態確認
#
# Usage:
#   bash scripts/qsub_gw_v3_batch.sh submit [--start N] [--end M] [--max_concurrent K]
#   bash scripts/qsub_gw_v3_batch.sh status
#   bash scripts/qsub_gw_v3_batch.sh submit_healthy
#
# Requires: jq, qsub (PBS)

DOE_FILE="${DOE_FILE:-doe_gw_v3.json}"
SCRIPT="scripts/qsub_gw_v3_single.sh"
...
if ! command -v jq &>/dev/null; then
    echo "ERROR: jq not installed. Install with: sudo yum install jq"
    exit 1
fi
...
count_running() {
    qstat -u "$USER" 2>/dev/null | grep "GW-v3" | grep -c " [RQ] " || echo 0
}
...
case "$ACTION" in
    status)
        ...
        for i in $(seq 0 $((N_TOTAL - 1))); do
            ...
            printf "  %-20s %-15s R=%-5s %s\n" "$JOB_NAME" "$DEFECT_TYPE" "$RADIUS" "$status"
        done
    ...
    submit)
        echo "Submitting v3 defective jobs (ID $START_ID to $END_ID, max_concurrent=$MAX_CONCURRENT)..."
        N_TOTAL=$(jq '.samples | length' "$DOE_FILE")
        submitted=0
        ...
        for i in $(seq 0 $((N_TOTAL - 1))); do
            ...
            # Build defect JSON (escape for PBS -v)
    ...
    submit_healthy)
        echo "Submitting v3 healthy jobs..."
        N_HEALTHY=$(jq '.healthy_samples | length' "$DOE_FILE")
        submitted=0
