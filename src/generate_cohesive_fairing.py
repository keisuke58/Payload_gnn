# -*- coding: utf-8 -*-
# generate_cohesive_fairing.py
# CZM (Cohesive Zone Model) adhesive layer implementation for H3 Type-S fairing.
#
# 5-part sandwich structure:
#   InnerSkin --Tie-- AdhesiveInner --Tie-- Core --Tie-- AdhesiveOuter --Tie-- OuterSkin
#                    (COH3D8 0.2mm)                     (COH3D8 0.2mm)
#
# Based on generate_realistic_dataset.py with Traction-Separation constitutive law
# for physically-based debonding simulation.
#
# Usage:
#   abaqus cae noGUI=generate_cohesive_fairing.py -- --job Job-CZM-Test --phase 2 --no_run
#   abaqus cae noGUI=generate_cohesive_fairing.py -- --job Job-CZM --param_file params.json
...
def generate_cohesive_model(job_name, defect_params=None, phase=2,
    ...
    """
    Main entry point: CZM cohesive fairing model with defect insertion.
    ...
    print("=" * 70)
    print("COHESIVE FAIRING MODEL (CZM) -- Phase %d" % phase)
    ...
    # 3. Partition openings (all 5 parts)
    if openings:
    ...
    #    Adhesive layers are excluded to preserve simple geometry for SWEEP.
    #    Defect assignment for adhesive uses cell centroid check instead.
    if defect_params:
        parts_to_partition = [
            ('shell', p_inner),
            ('solid', p_core),
    ...
    generate_mesh_with_adhesive(
        ...
        g_seed, o_seed, d_seed, f_seed, adh_t)
    ...
    print("=" * 70)
    print("DONE: %s (Phase %d, CZM adhesive=%.2fmm, %s)" % (
        job_name, phase, adh_t, defect_type or 'healthy'))
    print("=" * 70)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate CZM cohesive fairing model with defect insertion')
    parser.add_argument('--job', type=str, default='Job-CZM',
                        help='Job name')
