...
6. **A cross-structure calibration result** — five-structure leave-one-structure-out shows the detector's *ranking* transfers to a held-out structure (AUROC 0.61–1.00, mean 0.904) but a raw alarm *threshold* does not (false-alarm up to 0.997); per-structure self-normalisation with ~15 healthy measurements and one shared conformal threshold restores control (FPR ≤0.027 for discrete-damage structures). The DFOS stiffened panel (AUROC 0.61) reveals a **damage-type boundary**: diffuse fatigue accumulation defeats the residual detector, complementing the modality boundary (§5.5). §5.6.

---

## 2. Related Work

Structure-specific SHM methods dominate the literature. Guided-wave methods measure waveform changes along pitch-catch paths calibrated for a given panel thickness and layup [3]; vibration-based methods identify eigenfrequency shifts or mode-shape deviations for specific boundary conditions; strain-based methods threshold sensor readings against nominal values for a known load envelope. Each approach encodes geometry, sensing physics, and damage jointly into the same pipeline, so redeployment on a different structure requires redesigning the detector from scratch. The majority of published validations use a single structure type, a single sensing modality, and—critically—simulated rather than real experimental data.

Graph neural networks have emerged as a natural representation for mesh-based and sensor-network SHM. Pfaff et al. [1] demonstrated that message-passing GNNs trained on finite-element meshes reproduce physical dynamics across mesh resolutions, motivating their use as physics-informed surrogates. Dang et al. [2] applied graph-convolutional networks directly to vibration-sensor networks and showed improved damage detection over conventional thresholds on a single test structure. The present group's prior work [5] combined FEM-driven thermoelastic-stress fields with graph attention networks to achieve five-class defect localisation on CFRP panels. Across these studies, the graph is calibrated for one structure: node count, edge topology, and feature statistics are structure-specific, and no evaluation crosses material or modality boundaries.

Transfer learning and domain adaptation have been applied to SHM to handle distribution shifts between operating conditions, excitation frequencies, and nominally similar structures. Population-based SHM extends this further by sharing statistical models of healthy behaviour across a fleet. These approaches have shown consistent gains within a structure type. Cross-structure transfer—different material, different sensing modality, radically different geometry—is rarely attempted on real data, and negative results (where transfer fails and *why*) are rarely reported.

We occupy a distinct position. We forgo method novelty and instead ask whether *one* executable detection procedure, with only thin modality-specific adapters, produces valid detection AUROCs across five heterogeneous real structures spanning two material classes and seven sensing modalities. The contribution is a falsifiable, real-data, cross-structure benchmark with explicit failure modes—not a new estimator.
...
| Modality | `response` | `disp_mag` |
|---|---|---|
| DIC full-field (metallic panel) | von-Mises equiv. strain $\varepsilon_\text{vm}$ | $\|\mathbf{u}\| = \sqrt{u^2+v^2+w^2}$ |
| SLDV wavefield (omega-stringer) | per-frame spatial RMS of surface-normal velocity | masked |
| Thermo-elastic stress (interstage) | $\|\boldsymbol{\sigma}\| = \sqrt{\sigma_x^2+\sigma_y^2+\sigma_{xy}^2}$ | masked |
| Strain gauge — sparse (COPV burst) | $|\varepsilon|$ per gauge | masked |
| DFOS run-to-failure (stiffened panel) | $|\varepsilon|$ per distributed channel | masked |
| PZT guided wave (COPV GW) | per-path $\text{DI} = 1 - \text{corr}(\text{test}, \text{ref})$; aggregated per node over all paths touching that node | masked |
| Air-coupled UGW (sandwich panel) | per-channel $\max|\text{robust-}z(\text{amplitude})|$ after local linear detrending | masked |

The masked sentinel (zero) is ignored by the downstream detector so that displacement information is not hallucinated when the modality cannot provide it. The `response` sign convention is chosen so that damage always *increases* the score (e.g. load-shedding DFOS data uses $|{\varepsilon}|$ rather than raw $\varepsilon$).

**Detector (Stage-0).** For each node $i$ in frame $t$, the per-node anomaly score is

$$z_i(t) = \frac{|\text{response}_i(t) - \mu_i^{\text{ref}}|}{\sigma_i^{\text{ref}}}$$

where $\mu_i^{\text{ref}}$ and $\sigma_i^{\text{ref}}$ are the mean and standard deviation of `response` at node $i$ over healthy reference frames. When both `response` and `disp_mag` are non-masked, a bivariate Mahalanobis score replaces the univariate $z$. Frame-level detection aggregates $z_i$ across nodes (mean or $p$99); node-level detection reports $z_i$ directly. The detector has no learned parameters: it is a standardised residual against a per-structure healthy reference (canonical code: `tierb.py`, `stage0_detect.py`).

**Pipeline context.** This paper reports Stage-0 detection (§5) and partial Stage-3/4 prognosis (§6). The full pipeline—Stage 0 (detect) → 1 (classify/characterise) → 2 (locate) → 3 (prognosis) → 4 (fleet)—shares the Tier-B frame as its common input format.

---

## 4. Datasets — five real structures (the COPV under two modalities) + fairing sandwich proxy + prognosis source

Structures were selected to maximise **material × modality × damage-mode heterogeneity** subject to two constraints: (a) all data must be publicly available with permissive licences, making every claim independently reproducible; (b) at least one structure must provide real run-to-failure sequences for the §6 prognosis analysis. The COPV appears as two rows — sparse strain and 25-PZT guided wave — enabling the direct modality comparison of §5.5. The H3 fairing is absent (no real flight data released) and enters instead via two proxy structures (§5.4): the OGW omega-stringer and the TU Darmstadt real sandwich panel. All five primary datasets originate from the German aerospace-SHM community (DLR, BAM, TU Darmstadt), providing coherent provenance and inter-calibrated test practices.
...
| *Sandwich panel (fairing proxy, §5.4)* | *steel face / PIR foam* | *air-coupled UGW (12-ch)* | *12* | *core voids / foam-change (37)* | *TU Darmstadt doi:10.48328/tudatalib-2000* |
| CFRP multiaxial fatigue (prognosis only) | CFRP coupons | tabular S–N (biaxiality) | — | uni-/multi-axial loads | Mendeley jpk2t755vg |

Tier-B coverage (one frame each) — `response_cov`/`disp_cov`: metallic 1.00/1.00, omega-stringer 1.00/1.00, interstage 1.00/0.00 (masked), COPV strain 1.00/0.00, COPV GW 1.00/0.00, stiffened panel 1.00/0.00, sandwich 1.00/0.00. [R] The interstage displacement mask is correct: the DSPSS modality measures surface stress, not displacement field.
...
§5 detects each structure against *its own* healthy reference. The title's stronger promise — *structure-agnostic* — asks whether a detector calibrated on a set of structures transfers to a **held-out** one. We test this with leave-one-structure-out (LOSO) over **five** structures — metallic DIC, omega-stringer SLDV, COPV strain, the TU Darmstadt real sandwich (§5.4), and the ReMAP DFOS stiffened panel (CFRP run-to-failure, 5 specimens × multiple time points) — holding each out and calibrating the alarm on the others (`loso_calibration.py`, Fig. 8; the interstage is excluded, node-localisation structure, frame-AUROC 0.52). Within-structure AUROCs: 1.000 / 1.000 / 1.000 / 0.911 / **0.611** (DFOS stiffened, n_healthy = 2760 boot-frames).

| Held-out structure | AUROC | FPR (raw) | FPR (self-norm) | TPR (self-norm) |
|---|---|---|---|---|
| Metallic DIC | **1.000** | 0.000 | 0.000 | 0.808 |
| Omega-stringer | **1.000** | 0.000 | 0.000 | 1.000 |
| COPV strain | **1.000** | 0.000 | 0.000 | 0.000 |
| Sandwich (TU Darmstadt) | **0.911** | 0.249 | 0.027 | 0.393 |
| Stiffened panel (DFOS) | **0.611** | 0.109 | 0.214 | 0.368 |
| **mean** | **0.904** | 0.072 | 0.048 | 0.514 |

- **Ranking transfers across the first four structures.** Held-out AUROC ≥ 0.91 for metallic, omega, COPV, and sandwich — a detector calibrated on the others ranks damage on a never-seen structure, including the genuinely noisy sandwich. Mean AUROC over these four = 0.978.
- **DFOS stiffened panel: a new, honest detection boundary.** The ReMAP DFOS structure yields held-out AUROC **0.611**, barely above chance, and FPR_self **0.38** even with self-calibration. The Tier-B residual-based detector fails here because DFOS run-to-failure involves *gradual, distributed* strain accumulation across the whole panel — not the local concentration contrast the detector is designed for. This is not a failure of the contract but a new failure *mode*: the DFOS healthy regime shows large time-point-to-time-point and specimen-to-specimen variation (~2760 bootstrap frames) that overwhelms the damage signal at the chosen healthy/damaged split (frac_to_failure ≤0.25 vs ≥0.75). The structure is therefore an informative held-out that reveals a **damage-type boundary** (discrete local damage vs. diffuse fatigue accumulation) to complement the modality boundary in §5.5. [R]
- **A raw threshold does not transfer.** Score scales span orders of magnitude across structures (spatial-NN residual ~10⁻³ vs max-z ~5 for the sandwich), so one threshold from the N−1 pool is meaningless: held out, the sandwich false-alarms on **24.9 %** of healthy windows while the smaller-scale structures never alarm.
- **Self-calibration partially restores control.** Re-expressing each score in its own healthy units (median/IQR) and applying the *same* conformal threshold brings FPR to ≤ 0.027 for the sandwich, but **0.21 for the DFOS stiffened panel** — because the DFOS healthy variance is large and real (multi-specimen, multi-time-point), not a bootstrap artefact.

**Reading.** The five-structure LOSO consolidates the earlier four-structure result while adding a new data point: the Tier-B residual detector is structure-agnostic across metallic, composite-acoustic, and composite-guided-wave modalities, but fails at AUROC 0.61 for diffuse-damage DFOS run-to-failure. Structure-agnostic detection is achievable for discrete-damage structures; diffuse-damage run-to-failure requires a health-index rather than a residual (§6). *Honest scope:* false-alarm figures are node-bootstrap estimates; real healthy variability is carried by the metallic panel (three specimens), the sandwich (~400 panel windows), and the DFOS stiffened panel (5 specimens × multiple time points). The threshold-transfer claim is demonstrated on these three held-outs.
...
- ✅**LOSO クロス構造較正 統合（2026-06-21・§5.6新設・4構造→5構造）**：統一detector(各構造自分の健全ref残差)。metallic/omega/copv within-AUROC1.0＋TU Darmstadtサンド(air-coupled adapter)0.911＋**stiffened DFOS 5標本(run-to-failure)0.611**。**LOSO 5構造=mean AUROC 0.904(前4構造=0.978)。ranking転移(held-out AUROC 0.61-1.0)・生閾値は不転移(サンドFPR0.249)・自己正規化+共有conformal→離散損傷FPR≤0.027・DFOS FPR 0.214(真の多標本健全分散=高FPRの原因)**。DFOS=新 damage-type境界:「局所集中なき拡散疲労蓄積はTier-B残差では閾値近傍」。§6の正直な負と接続(RUL失敗の拡張形)。実健全変動=metallic(3標本)+サンド(~400窓)+DFOS(5標本×複数時点)の**3件が有意**。Abstract/§1貢献6/§7/§8/Fig8更新完了。
- **残（投稿前）**：(1) 英文最終コピーエディット、(2) 投稿先テンプレ整形(IWSHM2027→拡張SHM/MSSP)、(3) Fig8更新(5構造のbar追加)、(4) [任意] COPV GW 局在=FEMベース訓練ライブラリ(解析モデル失敗確認済→Abaqus COPV FEM活用)。両コピー同期済(`GNN/ingest`＝`Payload2026/papers`)。
