...
6. **A cross-structure calibration result** — leave-one-structure-out shows the detector's *ranking* transfers to a held-out structure (AUROC 1.000) but a raw alarm *threshold* does not (false-alarm 0.05→0.85); per-structure self-normalisation with ~15 healthy measurements and one shared conformal threshold restores it (→0.06) at 100 % detection. Structure-agnostic detection is achievable; structure-agnostic *thresholding* is not. §5.6.
...
§5 detects each structure against *its own* healthy reference. The title's stronger promise — *structure-agnostic* — asks whether a detector calibrated on a set of structures transfers to a **held-out** one. We test this with leave-one-structure-out (LOSO) on the three frame-level structures whose damage is a whole-frame event (metallic DIC, omega-stringer SLDV, COPV strain; the interstage is a node-localisation structure with a single healthy field, frame-AUROC 0.52, and is excluded). One detector — the Tier-B spatial-nearest-neighbour residual against each structure's own healthy reference, frame descriptor = mean residual — gives within-structure AUROC 1.000 on all three. Holding each out and calibrating the alarm on the others (`loso_calibration.py`, Fig. 8):
- **Ranking transfers.** Held-out AUROC = **1.000** for every structure: a detector tuned on the metallic panel and the CFRP vessel ranks damage on a never-seen CFRP stringer perfectly. Structure-agnostic *detection* holds.
- **A raw threshold does not.** A threshold set for 5 % false alarms on the *other* structures fires on **85 %** of healthy metallic frames — residual magnitudes are structure-specific, so an absolute threshold is meaningless across structures.
- **Self-calibration restores it.** Re-expressing the score in each structure's own healthy units (median/IQR) and applying the *same* conformal threshold drops the metallic false-alarm rate to **15 %**, and to **6 %** using only **15** held-out healthy sub-frames, while detection stays at **100 %**.

This makes the recurring observation of §5.4–5.5 (and the COPV-GW / OGW / sandwich proxies) explicit and cross-structural: **structure-agnostic detection is achievable, but structure-agnostic *thresholding* is not — a handful of healthy measurements on the new structure is the irreducible local ingredient.** *Honest scope:* the false-alarm figures are node-bootstrap estimates (sampling noise, not full operational variability); and only the metallic panel (three specimens) supplies real healthy variability, so omega-stringer and COPV — one healthy field each — have degenerate false-alarm rates. The threshold-transfer claim is therefore *demonstrated* on the metallic held-out and *suggested*, not proven, on the other two.
...
- **Detection is agnostic; thresholding is not (§5.6).** Leave-one-structure-out shows the detector's *ranking* transfers to an unseen structure (AUROC 1.000) but a *raw* alarm threshold does not (false-alarm 0.05→0.85 on the metallic held-out). Per-structure self-normalisation with ~15 healthy measurements plus one shared conformal threshold restores the false-alarm guarantee (→0.06) at full detection. The irreducible local ingredient is a handful of healthy measurements — not a re-designed detector.
...
- Fig8 **LOSO クロス構造較正**（§5.6）✅[生成済 2026-06-21]：`figs/fig8_loso_calibration.png`（左=FPR raw0.85→self-norm0.06／右=AUROC1.0転移）。生成元 `loso_calibration.py`・結果`loso_calibration_result.json`
...
- ✅**LOSO クロス構造較正 統合（2026-06-21・§5.6新設）**：統一detector(spatial-NN残差vs自分の健全ref)でmetallic/omega/copv within-AUROC1.0。**LOSO=ranking転移(held-out AUROC1.0)・生閾値は不転移(metallic FPR0.85)・自己正規化+共有conformalで回復(0.06,15健全サンプル,検出100%)**。「検出は構造非依存だが閾値較正は局所健全要」を横断実証。Abstract/§1貢献6/§7/Fig8/`loso_calibration.py`。正直scope: omega/copvは健全1フィールド=縮退、metallicのみ有意。interstageはnode-level(frame-AUROC0.52)で除外。
